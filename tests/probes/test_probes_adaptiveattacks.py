...
def test_adaptiveattacks_load(probename):
    p = garak._plugins.load_plugin(probename)
    assert isinstance(p, garak.probes.base.Probe)


@pytest.mark.parametrize("probename", ADAPTIVE_PROBES)
def test_adaptiveattacks_has_prompts(probename):
    p = garak._plugins.load_plugin(probename)
    assert len(p.prompts) > 0, "Probe must have at least one prompt"


@pytest.mark.parametrize("probename", ADAPTIVE_PROBES)
def test_adaptiveattacks_prompts_contain_template_markers(probename):
    p = garak._plugins.load_plugin(probename)
    sample = p.prompts[0]
...
def test_adaptiveattacks_respects_soft_probe_prompt_cap():
    cap = 5
    original_cap = _config.run.soft_probe_prompt_cap
    _config.run.soft_probe_prompt_cap = cap
    try:
        from garak.probes.adaptiveattacks import AdaptiveAttacks
